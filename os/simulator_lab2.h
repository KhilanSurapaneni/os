/*
 * simulator_2.h external declarations and stuff for the simulator.
 *
 */
...
int *InitArgsOnStack(int sp, char *argv[], int mem_base, int *new_sp);
int InitCRuntime(int sp, int *user_argv, char *argv[], int mem_base);
...
#include <stdbool.h>
#else
/* Pre-C99 fallback: define our own boolean type, but DON'T call it bool */
typedef enum
{
  FALSE = 0,
  TRUE = 1
...
/* Optional: if your old code uses bool/true/false identifiers,
   you can map them *as macros* in pre-C99 only. */
#ifndef bool
